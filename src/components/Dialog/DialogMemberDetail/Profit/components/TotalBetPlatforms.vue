...
import { apiQueryMemberPeriodPlatformBetAmount } from '@/api/dialogMemberDetail.js'
...
//取得資料
const queryMemberPeriodPlatformBetAmount = async () => {
  ...
  try {
    ...
    if (return_code === '0000') {
      ...
      transformMemberPeriodPlatformBetAmount(result.data.result)
...
//轉換資料
const transformMemberPeriodPlatformBetAmount = (data) => {
  clearChart()
...
onMounted(() => {
  queryMemberPeriodPlatformBetAmount()
})
</script>
<template>
