...
import { apiQueryGrowthRateActiveReal } from '@/api'
...
// 取得資料
const queryGrowthRateActiveReal = async () => {
  messageKey.value = 'loading'
  apiSuccess.value = false

  try {
    const result = await apiQueryGrowthRateActiveReal({
      hall_name: 'esx',
    ...
    if (return_code === '0000') {
      ...
      transformActivityMemberPeriodRealPeoplo(result.data.result)
...
//轉換資料
const transformActivityMemberPeriodRealPeoplo = (data) => {
  clearChart()

  let valueKey = 'real_activate_growth_rate'
