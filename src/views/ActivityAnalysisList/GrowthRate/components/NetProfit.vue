...
import { apiQueryGrowthRateActiveProfit } from '@/api'
...
// 取得資料
const queryGrowthRateActiveProfit = async () => {
  messageKey.value = 'loading'
  apiSuccess.value = false

  try {
    const result = await apiQueryGrowthRateActiveProfit({
      hall_name: 'esx',
    ...
    if (return_code === '0000') {
      ...
      transformActivityMemberPeriodNetProfit(result.data.result)
...
//轉換資料
const transformActivityMemberPeriodNetProfit = (data) => {
  clearChart()

  let valueKey = 'profit_loss_growth_rate'
