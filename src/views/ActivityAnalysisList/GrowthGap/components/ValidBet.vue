...
import { apiQueryGrowthGapActiveCommissionable } from '@/api'
...
// 取得資料
const queryGrowthGapActiveCommissionable = async () => {
  messageKey.value = 'loading'
  apiSuccess.value = false

  try {
    const result = await apiQueryGrowthGapActiveCommissionable({
      hall_name: 'esx',
    ...
    if (return_code === '0000') {
      ...
      transformActivityMemberPeriodGrowthGapBetAmount(result.data.result)
...
//轉換資料
const transformActivityMemberPeriodGrowthGapBetAmount = (data) => {
  clearChart()

  let valueKey = 'commissionable_growth_diff'
...
onMounted(() => {
  queryGrowthGapActiveCommissionable()
})
</script>
<template>
