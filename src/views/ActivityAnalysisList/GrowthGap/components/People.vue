...
import { apiQueryGrowthGapActiveReal } from '@/api'
...
// 取得資料
const queryGrowthGapActiveReal = async () => {
  messageKey.value = 'loading'
  apiSuccess.value = false

  try {
    const result = await apiQueryGrowthGapActiveReal({
      hall_name: 'esx',
    ...
    if (return_code === '0000') {
      ...
      transformActivityMemberPeriodGrowthGapRealPeoplo(result.data.result)
...
//轉換資料
const transformActivityMemberPeriodGrowthGapRealPeoplo = (data) => {
  clearChart()

  let valueKey = 'real_activate_growth_diff'
