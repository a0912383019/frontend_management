...
const queryTotalActiveReal = async () => {
  ...
  try {
    ...
    if (return_code === '0000') {
      ...
      transformActivityMemberPeriodTotalSumRealPeople(result.data.result)
...
//轉換資料
const transformActivityMemberPeriodTotalSumRealPeople = (data) => {
  clearChart()

  let valueKey = 'real_activate_sum'
