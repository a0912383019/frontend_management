...
import { apiQueryTotalActiveProfit } from '@/api'
...
// 取得資料
const queryTotalActiveProfit = async () => {
  messageKey.value = 'loading'
  apiSuccess.value = false

  try {
    const result = await apiQueryTotalActiveProfit({
      hall_name: 'esx',
    ...
    if (return_code === '0000') {
      ...
      transformActivityMemberPeriodTotalSumNetProfit(result.data.result)
...
//轉換資料
const transformActivityMemberPeriodTotalSumNetProfit = (data) => {
  clearChart()

  let valueKey = 'profit_loss_sum'
